...
(defun p2-tagbody-node (block target)
  (let* ((*blocks* (cons block *blocks*))
         ...
         (*register* *register*)
         (original-body (cdr (car (tagbody-form block))))
         (form (cdr (tagbody-form block)))
         (body (cdr form))
    ...
    (label BEGIN-BLOCK)
    (do* ((original-rest original-body (cdr original-rest))
          (original-subform (car original-rest) (car original-rest))
          (rest body (cdr rest))
          (subform (car rest) (car rest)))
         ((null rest))
      (cond ((or (symbolp original-subform) (integerp original-subform))
             (let ((tag (find original-subform (tagbody-tags block) :key #'tag-name
                              :test #'eql)))
               (unless tag
                 (error "COMPILE-TAGBODY: tag not found: ~S~%" original-subform))
               (when (tag-used tag)
                 (label (tag-label tag)))))
            (t
      ...
      (2
       (let* ((arg1 (%car args))
         ...
         (setf type1 (derive-compiler-type arg1)
               type2 (derive-compiler-type arg2)
               result-type (derive-compiler-type form))
         (cond ((and (node-constant-value-p arg1)
                     (node-constant-value-p arg2)
...
(define-derive-type-handler progn (form)
  (if (cdr form)
      (derive-compiler-type (car (last (cdr form))))
      NIL))
