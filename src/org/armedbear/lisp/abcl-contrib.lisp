...
(defun java.class.path ()
  "Return a list of the directories referenced in the JVM classpath

As a secondary value, return a list of the strings in the JVM classpath. "
  (let* ((separator (java:jstatic "getProperty" "java.lang.System" "path.separator"))
	 (path (coerce (java:jcall "split"
    ...
    (loop :for entry :in path
       :doing (let ((p (pathname entry)))
                (if (string-equal (pathname-type p) "jar")
                    (pushnew (make-pathname :defaults p :name nil :type nil)
                             result
                             :test 'equal)
                    (pushnew p
                             result
                             :test 'equal)))
       :doing (pushnew entry result :test 'equal))
    (values result result-raw)))
	   ...
	   (dolist (url (coerce (java:jcall "getURLs" entry)
        ...
        (t
         (format *standard-output*
                 "~&Skipping enumeration of resource ~a~%"
                 entry))))
      result)))
