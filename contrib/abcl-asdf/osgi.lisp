...
;; (jss:find-java-class <some class exported from your bundle>)
;; Do stuff
...
;; Class name can be abbreviated as with jss:find-java-class
...
;; (java:jstatic (find "loadLibrary"  (#"getDeclaredMethods"  (jss:find-java-class 'lang.classloader)) :key #"getName" :test 'equal)  c (jss:find-java-class 'FaCTPlusPlusReasonerFactory) "FaCTPlusPlusJNI" +false+)
...
(defun ensure-osgi-initialized (&key (empty-cache *osgi-clean-cache-on-start*))
  (unless *osgi-framework*
    (loop for function in *before-osgi-starting-hooks* do (funcall function))
    (let ((map (jss:new 'java.util.properties))
	...
	(java:add-to-classpath ;; sometimes resolve returns ":" separated pathnames of both main and framework jars. Only need the first.
	 ;; 5.6.1 current as of Jan/17
	 (car (uiop/utility:split-string (resolve "org.apache.felix/org.apache.felix.main/5.6.1") :separator '(#\:)))))
      (let* ((framework-factory-class (jss:find-java-class 'org.osgi.framework.launch.FrameworkFactory))
...
(defun force-unpack-native-libraries (bundle jar)
  ...
  (let ((wiring (#"adapt" bundle (jss:find-java-class 'BundleWiring))))
...
(defun index-class-names (names &key (table (make-hash-table :test 'equalp)))
  (jss:with-constant-signature ((matcher "matcher" t) (substring "substring")
			    (jreplace "replace" t) (jlength "length")
			    (matches "matches") 
			    (group "group"))
    (loop for name in names
	  with class-pattern = (java:jstatic "compile" "java.util.regex.Pattern" ".*\\.class{0,1}$")
	  with name-pattern = (java:jstatic "compile" "java.util.regex.Pattern" ".*?([^.]*)$")
...
(defun bundle-headers (bundle)
  (loop with headers = (#"getHeaders" bundle)
	for key in (jss:j2list (#"keys" headers))
...
(defun bundle-capabilities (bundle)
  (let ((bundleWiring (#"adapt" bundle (jss:find-java-class 'BundleWiring))))
    (loop with i = (#"iterator" (#"getCapabilities" bundlewiring java:+null+)) 
	  while (#"hasNext" i)
	  for cap = (#"next" i) 
	  for namespace = (#"getNamespace" cap)
	  for es = (#"entrySet" (#"getAttributes" cap)) 
	  collect (list* namespace cap (mapcar #"getValue" (jss:set-to-list es))))))
...
(defun bundle-exports (bundle)
  (let ((entry (bundle-header bundle "Export-Package"))
	(bundleWiring (#"adapt" bundle (jss:find-java-class 'BundleWiring))))
	...
	(loop for package-prefix
		...
		(loop with candidates = (sort (mapcar (lambda(el) (#"replaceAll" el ";.*$" "")) 
						      (uiop/utility::split-string (#"replaceAll" entry "(\\\".*?\\\")" "") :separator '(#\,)))			 
					      'string-lessp)
	      ...
	      append
	      (loop for entry in (jss:set-to-list (#"listResources" bundlewiring (concatenate 'string "/" path)
								"*.*" (java:jfield (jss:find-java-class 'BundleWiring) "FINDENTRIES_RECURSE")))
	...
	(loop for entry in (jss:set-to-list (#"listResources"
					 bundlewiring "/"
					 "*.*" (java:jfield (jss:find-java-class 'BundleWiring) "FINDENTRIES_RECURSE")))
	      for url = (#"toString" (#"getEntry" bundle entry))
...
(defun dwim-find-bundle-entry (name)
  (let ((string (string name)))
    (let ((candidates (remove-if-not (lambda (e) (search string (car e) :test 'string-equal))
                                     *loaded-osgi-bundles*)))
...
;; look for it like jss:find-java-class and don't cache it for
;; jss:find-java-class. Default currently is to do so, but I might change
;; the default, as it could lead to confusion in the case where both
;; jss:find-java-class and find-bundle-class are used and there are two
...
(defun find-bundle-class (bundle classname &key no-cache &aux bundle-entry)
  ...
  (or (let ((found (and (not no-cache) (gethash (string classname) jss:*imports-resolved-classes*))))
	(and (consp found) (eq (car found) bundle) (second found)))
      (let ((found (jss:lookup-class-name classname :table (third bundle-entry))))
	(if found
	    (progn 
	      (unless no-cache
		(unless (gethash classname jss:*imports-resolved-classes*)
		  (setf (gethash classname jss:*imports-resolved-classes*) (cons bundle found))))
