...
Following a fecund Spring, the Bear has consumed a frugal diet of
java.nio.Buffer objects for its basic abstractions of arrays
specialized on (or (unsigned-byte 8) (unsigned-byte 16) (unsigned-byte
32)).  Not all parts of the implementation were complete, but the
basic shape of the code was deemed worthy to bless as abcl-1.7.0 in
June 2020.
...
\texttt{COPYING} in the source distribution\footnote{See
  \url{http://abcl.org/svn/trunk/tags/1.7.0/COPYING}} for the license,
...
\texttt{abcl.jar} or possibly something like \texttt{abcl-1.7.0.jar} if
...
See \code{JAVA:JNEW-RUNTIME-CLASS} in \ref{JAVA:JNEW-RUNTIME-CLASS}.
...
\code{java}, \code{threads}, or \code{extensions} packages.
...
\begin{description}
  ...
  \item{\code{ABCL-CONTRIB}} 
    Locates and pushes the top-level contents of
    ...
    \begin{enumerate}
      ...
      \item \code{mvn} 
        ...
        \begin{enumerate}
          \item \code{jna} 
            Dynamically load 'jna.jar' version 5.5.0
...
asdf-3.3.3 (see \cite{asdf}) is packaged as core component of \textsc{ABCL},
...
When the implementation uses subclasses of \code{java.nio.Buffer} for
its storage of vectors and arrays specialized on the byte-vector
types\footnote{Available starting in abcl-1.7.0 and indicated by the
presence of \code{:nio} in \code{cl:*features*}}:
...
the following additional keywords are added to \code{cl:make-array}:
...
\item[\code{:nio-direct GENERALIZED-BOOLEAN}]

When \code{GENERALIZED-BOOLEAN} is true, constructs a ``direct''
underlying nio buffer.  The buffers returned by this method typically
have somewhat higher allocation and deallocation costs than non-direct
...
As an example, the following would create a 16 byte vector using the
created byte-buffer for storage:
\begin{listing-lisp}
  (let* ((length 16)
         (byte-buffer (java:jstatic "allocate" "java.nio.ByteBuffer" 16)))
    (make-array 16 :element-type '(unsigned-byte 8) :nio-buffer byte-buffer))
\end{listing-lisp}
...
\code{abcl-contrib-1.7.0.jar}.  The contrib jar is not loaded by the
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/abcl-asdf/README.markdown}.
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/asdf-jar/README.markdown}.
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/jss/README.markdown}
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/jfli/README}.
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/abcl-introspect/}.
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/abcl-build/}.
...
\url{http://abcl.org/svn/tags/1.7.0/contrib/named-readtables/}.
