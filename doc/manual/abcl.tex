...
We implement a comprehensive extension to the \code{CL:PATHNAME} that
...
\code{sun.net.protocol.dynamic} package. See \cite{maso2000} for more
details.
...
may be broadly used anywhere a \code{CL:PATHNAME} is accepted with the
general caveat that stream obtained via \code{CL:OPEN} on either subtype
...
\subsubsection{Implementation of a JAR-PATHNAME}
  ...
  \code{DEVICE} possibly be a list of pathnames.  objects.  The first
  member of this list will be a \code{EXT:URL-PATHNAME} pointing either to a
  ...
  of the list contains ``traditional'' \code{CL:PATHNAME} objects
  ...
  of one or more elements.  If \code{DEVICE} is a list, it denotes a
  \code{EXT:JAR-PATHNAME}.  Passing any pathname with a \code{DEVICE}
  that is such a list to \code{CL:TRUENAME} will return a object whose
  Lisp type is \code{EXT:JAR-PATHNAME}, and whose \code{DEVICE} will
  contain references to objects normalized to the following
  gcontraints.  The first entry in the \code{DEVICE} list of
  \code{EXT:JAR-PATHNAME} will be a \code{EXT:URL-PATHNAME}
  ...
  the list contain simple \code{CL:PATHNAME} objects representing
...
baseline.  This is intended as the last major release to support
openjdk6, openjdk7, and openjdk8.
