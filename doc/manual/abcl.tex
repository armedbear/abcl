...
zipfile now work for most read-only operations (\code{CL:PROBE-FILE},
\code{CL:TRUENAME}, \code{CL:OPEN}, \code{CL:LOAD},
\code{CL:FILE-WRITE-DATE}, \code{CL:DIRECTORY}, and
\code{CL:MERGE-PATHNAMES}).  The previous version relied on the
ability for \code{java.net.URL} to open streams of an archive within
...
superset of what is commonly understood as a \textsc{URL}. We
...
addressing of arbitrarily recursive entries within ZIP archives.
These implementation decisions are encapsulated by the specialization
of \code{CL:PATHNAME} as the \code{EXT:URL-PATHNAME}
...
implementation of \code{java.net.URL} to access (minimally) resources
named by the ``file'', ``http'', ``https'', and ``ftp'' schemes.
...
details.}.  The namestring of a \code{EXT:URL-PATHNAME} specialization
is equivalent to the string serialization of its representation as defined.
...
is a superset.  \footnote{JAR archive utilize the ZIP format for
packing and compression adding procedures to add supporting metadata
in a manifest which is standarized text file stored at a canonical
...
\code{CL:OPEN} on either subtype cannot be the target of write
operations.
...
\code{EXT:URL-PATHNAME} pointing either to a source of bytes on the
local filesystem or as a remote source via an stream-oriented
messaging protocol such as \textsc{https}.  The rest of the list
contains ``traditional'' \code{CL:PATHNAME} objects denoting
successive relative archive paths.

Passing any pathname with a \code{DEVICE} that is such a list to
\code{CL:TRUENAME} will return a object whose Lisp type is
\code{EXT:JAR-PATHNAME}, and whose \code{DEVICE} will contain
references to objects normalized to the following contraints.  The
first entry in the \code{DEVICE} list of \code{EXT:JAR-PATHNAME} will
be a \code{EXT:URL-PATHNAME} designating the root source of bytes for
root archive compressed via the ZIP compression algorithm.  If any,
the remaining elements of the list contain simple \code{CL:PATHNAME}
objects representing subsequent entries objects \footnote{This allows
for reference to arbitrarily nested ZIP archives, which is the case
when the an ABCL fasl is included in in a jar archive.}

  %% citation from CLHS needed
...
\code{EXT:JAR-PATHNAME}.  We extend the semantics for the usual
PATHNAME merge semantics when \code{*DEFAULT-PATHNAME-DEFAULTS*}
...
encapsulate succesive locations.  Described broadly, a
\code{EXT:JAR-PATHNAME} contains a \textsc{URL} describing the location of the
archive and a entry within that archive.
...
Subsequent entries within an archive are denoted as 
\begin{verbatim}
